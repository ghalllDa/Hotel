            ...
            <tbody>
                @foreach($hotels as $hotel)
                    <tr>
                        ...
                        <td class="border px-4 py-2">Rp {{ number_format($hotel->harga) }}</td>
                        <td class="border px-4 py-2">{{ $hotel->fasilitas }}</td>
                        <td class="border px-4 py-2">
                            @if($hotel->gambar)
                                <img src="{{ asset('storage/'.$hotel->gambar) }}" class="w-20 h-20 object-cover rounded">
                            @endif
                        </td>
                        <td class="border px-4 py-2">
                            ...
                            <form action="{{ route('hotels.destroy', $hotel) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600" onclick="return confirm('Yakin mau dihapus?')">Hapus</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
